...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>股票代码：</div>
        <el-input v-model="formSearch.gupiaodaima" placeholder="股票代码" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>股票名称：</div>
        <el-input v-model="formSearch.gupiaomingcheng" placeholder="股票名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('gupiaoxinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 10px","margin":"0px 10px 0 0","color":"#333","borderRadius":"2px","background":"#0062a530","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/gupiaoxinxiAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"padding":"0 0 5px","margin":"0","borderColor":"#ccc","overflow":"hidden","textAlign":"right","background":"none","borderWidth":"0 0 1px","width":"100%","borderStyle":"dotted","order":"4"}'>
		<el-button :style='{"border":"0px solid #ddd","padding":"2px 10px","margin":"0 5px","borderRadius":"0px","background":"none"}' @click="sortClick('clicknum')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#0062a5"}' class="icon iconfont icon-xiaoliang17" v-if="sortType!='clicknum'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#0062a5"}' class="icon iconfont icon-jiantou06" v-else-if="sortType=='clicknum'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#0062a5"}' class="icon iconfont icon-jiantou07" v-else-if="sortType=='clicknum'&&sortOrder=='asc'"></span>
			<span :style='{"color":"#25a7ff","lineHeight":"20px","fontSize":"14px"}'>点击量</span>
		</el-button>
	</div>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0 0 10px","margin":"10px 0 0","borderColor":"#cbcbcb","background":"#fff","borderWidth":"0 0 1px","display":"flex","width":"100%","fontSize":"0","position":"relative","borderStyle":"dotted","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div :style='{"border":"0px solid #ddd","width":"160px","padding":"0px","overflow":"hidden","height":"100px"}'>
					<img @click.stop="imgPreView(item.gupiaotupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.gupiaotupian && item.gupiaotupian.substr(0,4)=='http'" :src="item.gupiaotupian.split(',')[0]" class="image" />
					<img @click.stop="imgPreView(baseUrl + (item.gupiaotupian?item.gupiaotupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.gupiaotupian?item.gupiaotupian.split(',')[0]:'')" class="image" />
				</div>
				<div class="item-info" :style='{"padding":"10px","overflow":"hidden","alignItems":"center","color":"#666","flexWrap":"wrap","flex":"1","background":"none","display":"flex","fontSize":"14px","height":"auto"}'>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">股票代码:{{item.gupiaodaima}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">股票名称:{{item.gupiaomingcheng}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">股票类型:{{item.gupiaoleixing}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">历史最高价:{{item.lishizuigaojia}}</div>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">历史最低价:{{item.lishizuidijia}}</div>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/gupiaoleixing/gupiaoleixing').then(res => {
			if (res.data.code == 0) {
				this.gupiaoleixingOptions = res.data.data;
				this.selectOptionsList.push({name:'股票类型',list:this.gupiaoleixingOptions,tableName: 'gupiaoleixing',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.gupiaodaima != '') searchWhere.gupiaodaima = '%' + this.formSearch.gupiaodaima + '%';
        if (this.formSearch.gupiaomingcheng != '') searchWhere.gupiaomingcheng = '%' + this.formSearch.gupiaomingcheng + '%';
        if (this.formSearch.gupiaoleixing != '') searchWhere.gupiaoleixing = this.formSearch.gupiaoleixing;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`gupiaoxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/gupiaoxinxiDetail', query: params});
      },
