<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0px 0px 20px","overflow":"hidden","color":"#666","background":"#e5e6e6","fontSize":"14px","position":"relative"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","background":"none","width":"auto","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px","borderColor":"#355749","color":"#c2c2c2","textAlign":"center","flexDirection":"row","display":"flex","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						...
						<el-input v-model="searchForm.stockcode" placeholder="股票代码" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px 20px 10px","margin":"20px 0 0","borderColor":"#355749","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button class="add" v-if="isAuth('topten','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('topten','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('topten','爬取数据')" type="success" @click="spider()">
						<span class="icon iconfont icon-shuju17" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						爬取数据
					</el-button>

					<el-button class="btn18" v-if="isAuth('topten','收盘价')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						收盘价
					</el-button>
					<el-button class="btn18" v-if="isAuth('topten','涨跌额')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						涨跌额
					</el-button>
					<el-button class="btn18" v-if="isAuth('topten','买入金额')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						买入金额
					</el-button>
					<el-button class="btn18" v-if="isAuth('topten','卖出金额')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						卖出金额
					</el-button>
					<el-button class="btn18" v-if="isAuth('topten','成交金额')" type="success" @click="chartDialog5()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						成交金额
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #e1e8ee","padding":"0px","margin":"0px auto","borderColor":"#355749","borderRadius":"4px","background":"rgba(255,255,255,1)","borderWidth":"2px 0 0","width":"calc(100% - 40px)","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('topten','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('topten','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('topten','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="closingpriceChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="pricerangeChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="buybalanceChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="sellbalanceChart4" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="balanceChart5" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var closingpriceChart1 = echarts.init(document.getElementById("closingpriceChart1"),'macarons');
        this.$http({
            url: `topten/value/stockname/closingprice`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].stockname);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].stockname
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                closingpriceChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    closingpriceChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var pricerangeChart2 = echarts.init(document.getElementById("pricerangeChart2"),'macarons');
        this.$http({
            url: `topten/value/stockname/pricerange`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                pricerangeChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    pricerangeChart2.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var buybalanceChart3 = echarts.init(document.getElementById("buybalanceChart3"),'macarons');
        this.$http({
            url: "topten/group/buybalance",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].buybalance);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].buybalance
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                buybalanceChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    buybalanceChart3.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog4() {
      ...
      this.$nextTick(()=>{
        ...
        var sellbalanceChart4 = echarts.init(document.getElementById("sellbalanceChart4"),'macarons');
        this.$http({
            url: "topten/group/sellbalance",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].sellbalance);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].sellbalance
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                sellbalanceChart4.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    sellbalanceChart4.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog5() {
      ...
      this.$nextTick(()=>{
        ...
        var balanceChart5 = echarts.init(document.getElementById("balanceChart5"),'macarons');
        this.$http({
            url: "topten/group/balance",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                balanceChart5.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    balanceChart5.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.stockcode!='' && this.searchForm.stockcode!=undefined){
            params['stockcode'] = '%' + this.searchForm.stockcode + '%'
          }
    params['sort'] = 'ranking';
    params['order'] = 'asc';
